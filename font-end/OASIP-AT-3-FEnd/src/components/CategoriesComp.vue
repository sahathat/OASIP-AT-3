...
//Delete Owner
const toDeleteOwner = async (email) => {
  const key = localStorage.getItem('key')
  const res = await fetch(`${mappingLink}/${params.id}`, { 
    method: "DELETE",
    headers: {
            "Authorization":'Bearer ' + key ,
            "Accept": 'application/json',
  ...
  if (res.status === 200) {
    ...
    Swal.fire('Delete Successful !', 'This Lecturer Owner has been deleted.','success')
    getDetail()
  }
  // refresh token ----------------------------- //
    else if (res.status === 401 && localStorage.getItem('token')==='accessToken') {
    ...
    // console.log("delete successfully");
      Swal.fire('Delete Fail !', 'This Category must have at least 1 owner.','error')
    }
    // ------------------------------------------------- // 
...
<template>
<body>
  <div v-if="isNotNull == true"  class="container rounded bg-white mt-1 mb-5">
    <div class="row">
        <div class="col-md-2 border-right"></div>
        <div class="col-md-5 border-right">
            <div class="p-3 py-5">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h2 class="text-right">Category Detail</h2>
                </div>
                <div class="row mt-2">
                    <div class="col-md-12">
                      <label class="labels fw-semibold">Category Name :</label>
                      <input type="text" class="form-control" :value="`${name}`" disabled>
                    </div>
                </div>
                <div class="row mt-3">
                    <span class="col-md-12">
                      <label class="labels fw-semibold"> Duration : </label>
                      <input type="text" class="form-control" :value="`${duration}   minutes`" disabled>
                    </span>
                </div>
                <div class="row mt-3">
                    <div class="col-md-12">
                      <label class="labels fw-semibold">Description :</label>
                      <textarea class="form-control" :value="`${description}`" disabled></textarea>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-4 mt-5">
            <div class="p-3 py-5">
                <div class="d-flex justify-content-between align-items-center experience">
                  <span class="fw-semibold">Lecturer Owner :</span>
                </div>
                <div class="col-md-12">
                  <p class="fw-semibold text-start" style="padding-left: 20px;padding-right: 20px;">
                      <ol>
                        <li v-for="lecturer in lecturerOnCategory" :key="lecturer.id">
        ...
        <div class="mt-2 text-center">
            <button v-if="userRole=='admin'" class="btn btn-warning col-md-1" type="button" data-bs-target="#editDetail" data-bs-toggle="modal" @click="editInfo">Edit</button>
            <button class="btn btn-secondary col-md-1" type="button" style="margin-left: 10px;" @click="goCategoriesList()">Back</button>
        </div>
      </div>
  </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                                    ...
                                    <div style="margin-top: 20px;">
                                        <div class="px-2">
                                            ...
                                            <p class="fw-semibold text-start" style="padding-left: 20px;padding-right: 20px;">
                                              <ol>
                                                <li v-for="lecturer in lecturerOnCategory" :key="lecturer.id">
                                                  <span class="fw-bold" style="font-size: 18px;">{{ lecturer.name }} 
                                                    <button v-if="userRole=='admin'" class="btn btn-danger btn-sm float-end" type="button" style="margin-left: 10px;" @click="toDeleteOwner(lecturer.email)" > delete </button> <br>
                                                  </span>
                                                  <span class="fw-semibold" style="font-size: 16px;">email: {{ lecturer.email }} <br></span>
                                                </li>
                                              </ol>
                ...
                <div class="modal-footer">
                  ...
                  <button class="btn btn-primary" type="button" style="background: #20c997;" data-bs-target="#submit" data-bs-toggle="modal">Save</button>
                </div>
            </div>
        </div>
    </div>
